...
void qmp_dirty(Monitor *mon,const char *uri, bool has_blk, bool blk,
                 bool has_inc, bool inc, bool has_detach, bool detach,
                 bool has_resume, bool resume, Error **errp)
{
    Error *local_err = NULL;
...
void dirty_fd_connect(Monitor *mon,MigrationState *s, Error *error_in)
{
    int64_t rate_limit;
    bool resume = s->state == MIGRATION_STATUS_POSTCOPY_PAUSED;
    ...
    monitor_printf(mon,"[LOG] IN dirty_fd_connect \n");
    if (error_in) {
        migrate_fd_error(s, error_in);
        migrate_fd_cleanup(s);
        return;
    ...
    qemu_thread_create(&s->thread, "live_migration", dirty_thread, s,
                       QEMU_THREAD_JOINABLE);
    s->migration_thread_running = true;
}
